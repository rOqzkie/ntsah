        ...
        <div class="col-md-9">
            <div class="card card-outline shadow rounded-0">
                <div class="card-body rounded-0">
                    <h2 class="card-body rounded-0 bg-primary">Archive(s)</h2>
                    <hr class="bg-navy">
                    ...
                    //$query = "SELECT * FROM archive_list WHERE `status` = 1 {$search} {$yearFilter} {$whereClause} ORDER BY {$sortOrder} {$paginate}";
                    $query = "SELECT a.*, c.description AS college_name 
                                FROM archive_list a 
                                LEFT JOIN curriculum_list cu ON a.curriculum_id = cu.id
                                LEFT JOIN college_list c ON cu.college_id = c.id 
                    ...
                    <div class="list-group">
                        <?php while($row = $archives->fetch_assoc()): 
                            $row['title'] = preg_replace("/{$keyword}/i", $highlight, $row['title']);
                            $row['abstract'] = preg_replace("/{$keyword}/i", $highlight, strip_tags(html_entity_decode($row['abstract'])));
                        ?>
                ...
                <div class="col-12">
                    <div class="row">
                        ...
                        <div class="col-md-6 text-left">
                            <span class="text-muted">Displaying <?= $archives->num_rows ?> of <?= $totalRecords ?> Item(s)</span>
                        </div>
                        <div class="col-md-6">
                            <ul class="pagination pagination-sm m-2 float-right">
                                <li class="page-item">
                                    <a class="page-link" href="./?page=archives<?= $isSearch ?>&p=<?= $page - 1 ?>" <?= $page == 1 ? 'disabled' : '' ?>>«</a>
                                </li>
                                <?php for($i = 1; $i<= $pages; $i++): ?>
                                    <li class="page-item">
                                        <a class="page-link <?= $page == $i ? 'active' : '' ?>" href="./?page=archives<?= $isSearch ?>&p=<?= $i ?>"><?= $i ?></a>
                                    </li>
                                <?php endfor; ?>
                                <li class="page-item">
                                    <a class="page-link" href="./?page=archives<?= $isSearch ?>&p=<?= $page + 1 ?>" <?= $page == $pages ? 'disabled' : '' ?>>»</a>
                                </li>
                            </ul>
                        </div>
                    </div>
