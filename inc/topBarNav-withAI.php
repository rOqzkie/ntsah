        ...
        <div class="top-bar">
            ...
            <div class="right-links">
                <?php if($_settings->userdata('type') == 3): ?>
                    ...
                    <span class="mx-1"><a href="<?= base_url.'classes/Login.php?f=adviser_logout' ?>"><i class="fa fa-power-off"></i></a></span>
                <?php endif; ?>
                <?php if($_settings->userdata('type') == 2): ?>
                    <a href="./?page=profile"
                    <span class="mx-2"><img src="<?= validate_image($_settings->userdata('avatar')) ?>" alt="User Avatar" id="student-img-avatar" title="Profile"></span>
                    </a>
                    <span class="mx-2">Welcome, <?= !empty($_settings->userdata('firstname')) ? $_settings->userdata('firstname')." ".$_settings->userdata('lastname') : $_settings->userdata('username') ?></span>
                    <span class="mx-1"><a href="<?= base_url.'classes/Login.php?f=student_logout' ?>"><i class="fa fa-power-off" title="Logout"></i></a></span>
                <?php endif; ?>
            </div>
        </div>
        <nav>
            <div class="logo">📂 NEMSU Archiving Hub</div>
            <ul class="nav nav-pills align-items-center">
                <?php if ($_settings->userdata('type') == 3): ?>
                <li class="nav-item">
                    <a href="./?page=submit-archive" 
                       class="nav-link btn btn-primary text-white px-4 py-2 rounded-pill fw-bold shadow-sm 
                              <?= isset($page) && $page == 'submit-archive' ? "active" : "" ?>" 
                       id="submit-btn">
                        <i class="fas fa-upload me-2"></i> Submit Thesis/Feasibility Study
                    </a>
                </li>
                <?php endif; ?>
                <li class="nav-item">
                    <a href="./" class="nav-link" title="Home">
                        <img src="./uploads/archive.png" class="nav-icon" alt="Archive">
                    </a>
                </li>
            </ul>
        </nav>
        <div class="banner">
            <h1 id="slideshow">Empowering Research, Preserving Knowledge</h1>
            <?php
                $totalarchives = $conn->query("SELECT * FROM archive_list where status = 1")->num_rows;
            ?>
            <p>SEARCH <span class="green"><?php echo $totalarchives; ?></span> TOTAL NO. OF ARCHIVES</p>
...
$(document).ready(function(){
    ...
    function performSearch() {
        ...
        let selectedCollege = $('#college-select').val();
        let currentPage = new URLSearchParams(window.location.search).get("page");
        let url = "./?page=" + (currentPage === "projects_per_college" ? "projects_per_college" : "projects");
        ...
        debounceTimer = setTimeout(() => {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    let suggestionList = suggestions.map(item => `<div class="suggestion-item">${item}</div>`).join('');
                    $('#suggestions-box').html(suggestionList).show();
                }
            });
        }, 300);
    }
    ...
    if (selectedCollege) $('#college-select').val(selectedCollege);
    if (query) $('#search-input').val(query);
});
$(document).ready(function(){
