...
<div class="container-fluid">
    <form action="" id="department-form">
        ...
        <div class="form-group">
            <label for="college_id" class="control-label">College</label>
            <select name="college_id" id="college_id" class="form-control form-control-border" required data-placeholder="Select College Here">
                <option <?= !isset($college_id) == 1 ? "selected" :"" ?>></option>
                <?php 
                $college = $conn->query("SELECT * FROM `college_list` where `status` = 1 ".(isset($college_id) ? "OR id = '{$college_id}'" : "")." order by `name` asc");
                while($row = $college->fetch_assoc()):
                ?>
                <option value="<?= $row['id'] ?>" <?= isset($college_id) && $college_id == $row['id'] ? "selected" : "" ?>><?= ucwords($row['name']) ?></option>
                <?php endwhile; ?>
            </select>
        </div>
        <div class="form-group">
