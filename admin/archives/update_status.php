    ...
    <form action="" id="update_status_form">
        ...
        <div class="form-group">
            ...
            <select name="status" id="status" class="form-control form-control-border" required>
                <option value="0" <?= isset($_GET['status']) && $_GET['status'] == 0 ? "selected" : "" ?>>For Archiving Approval</option>
                <option value="1" <?= isset($_GET['status']) && $_GET['status'] == 1 ? "selected" : "" ?>>Approve to Archive</option>
            </select>
        </div>
    </form>
</div>
